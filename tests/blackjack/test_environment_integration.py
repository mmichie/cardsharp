...
class TestEnvironmentIntegration(unittest.TestCase):
    ...
    def test_add_player(self):
        ...
        self.assertEqual(player.strategy.__class__.__name__, "ExecutionVarianceWrapper")
        self.assertEqual(player.strategy.strategy.__class__.__name__, "RealisticPlayerStrategy")
    ...
    def test_simulate_session(self, mock_time):
        ...
        cursor.execute("SELECT COUNT(*) FROM sessions WHERE session_id = ?", 
                      (self.session_id,))
        count = cursor.fetchone()[0]
        self.assertEqual(count, 1)
        
        # Check round records
        cursor.execute("SELECT COUNT(*) FROM rounds WHERE session_id = ?", 
                      (self.session_id,))
    ...
    def test_dealer_errors(self):
        ...
        dealer = DealerProfile(
            name="Error Dealer",
            error_rate=1.0  # Always makes errors
        )
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM events WHERE event_type = 'dealer_error'"
        )
        count = cursor.fetchone()[0]
        self.assertEqual(count, 1)
