    ...
    def parse_game(notation: str) -> GameRecord:
        ...
        if player_part.startswith("P:"):
            ...
            for hand_str in hand_strs:
                cards_str, actions_str = (
                    hand_str.split(":") if ":" in hand_str else (hand_str, "")
                )
                cards = BlackjackNotation.parse_card_list(cards_str)
                actions = (
                    BlackjackNotation.parse_action_sequence(actions_str)
                    if actions_str
                    else []
                )
                player_hands.append(
                    Hand(initial_cards=cards, actions=actions, is_split=True)
                )
        else:
            # Single hand
            cards_str, actions_str = (
                player_part.split(":") if ":" in player_part else (player_part, "")
            )
            cards = BlackjackNotation.parse_card_list(cards_str)
            actions = (
                BlackjackNotation.parse_action_sequence(actions_str)
        ...
        # Parse dealer
        dealer_cards_str, dealer_actions = (
            dealer_part.split(":") if ":" in dealer_part else (dealer_part, "")
        )
...
STANDARD_TESTS = [
    ...
    TestCase(
        ...
        deck=[
            Card.from_str(c) for c in ["8h", "8d", "Th", "7s", "3c", "As", "2h", "9d"]
        ],
